...
for (( i = 0; i < ${iterations}; i++ )); do
    ...
    ssh root@${etcdHostname} "cp ${logFiles} ${recovPath}; tar -xzvf ${recovPath}/logs.tar.gz -C ${recovPath} --strip-components 3; rm ${recovPath}/logs.tar.gz"
    ...
    ssh root@${etcdHostname} "killall etcd -u root -w; mv ${measureFile} ${rootFolder}/${i}-recov-time.out; rm ${recovPath}/*"
